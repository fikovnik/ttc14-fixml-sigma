...
trait ClassScalaSupport extends EMFScalaSupport {
  ...
  object Class {
    def apply(name: String = null, abstract_ : Boolean = false, superclass: fr.inria.spirals.sigma.ttc14.fixml.objlang.Class = null, members: EList[Member] = null): Class = {
    ...
    def unapply(that: Class): Option[(String,Boolean,fr.inria.spirals.sigma.ttc14.fixml.objlang.Class,EList[Member],EList[Constructor],EList[Field])] =
      Some((that.getName,that.isAbstract,that.getSuperclass,that.getMembers,that.getConstructors,that.getFields))
  }
}
